...
public class ShapeDataDrawer : Editor
{
    ...
    private void DrawBoardTable()
    {
        ...
        var dataFiledStyle = new GUIStyle(EditorStyles.miniButtonMid);
        dataFiledStyle.normal.background = Texture2D.grayTexture;
        dataFiledStyle.onNormal.background = Texture2D.whiteTexture;
        ...
        for(var row = 0; row < ShapeDataInstance.rows; row++)
        {
            ...
            for(var column = 0; column < ShapeDataInstance.columns; column++)
            {
                ...
                var data = EditorGUILayout.Toggle(ShapeDataInstance.board[row].column[column], dataFiledStyle);
